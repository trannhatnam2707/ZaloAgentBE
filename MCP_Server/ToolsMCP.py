...
from Services.Report_service import create_report as create_report_service # Import service
...
# =====================================================
# 🧠 Tool: Phân tích ý định của người dùng (Đã xóa tóm tắt)
# =====================================================
...
def save_report_tool(username: str, date: str, yesterday: str = None, today: str = None, 
    ...
    """
    # (Toàn bộ logic của hàm này được giữ nguyên như file của bạn)
    try:
        ...
        if report:
            ...
            if yesterday: update_data["yesterday"] = yesterday.strip()
            if today: update_data["today"] = today.strip()
            if date and date != report["date"]: update_data["date"] = date
            if new_date: update_data["date"] = new_date
            ...
            if not all([yesterday, today]):
                 return {"success": False, "error": "Thiếu thông tin 'yesterday' hoặc 'today' để tạo báo cáo."}
